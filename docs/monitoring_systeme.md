...
**Distinction avec C11, à ne pas confondre** : `docs/MONITORING.md` (preuve pour C11) documente le
monitorage du **modèle** — ses métriques de performance (F1, accuracy) exposées via MLflow. Ce
document-ci couvre le monitorage du **système** — est-ce que l'application elle-même tourne
correctement (disponibilité, erreurs, latence, dépendances externes), indépendamment de la
qualité des prédictions du modèle. Les deux s'appuient sur la même stack technique
(Prometheus/Grafana) mais répondent à des questions différentes. Précision utile : la mention
d'une "feedback loop" dans l'intitulé du critère C20 de la grille RNCP renvoie en réalité à une
logique de MLOps (réinjection de données vers le réentraînement du modèle, cf. C11), pas au
...
dashboards et pour évaluer en continu 3 règles d'alerte (moteur d'alerting intégré à Grafana,
...
Les 3 seuils sont volontairement peu nombreux (le projet n'en a pas besoin de 50) et rattachés
...
`docker-compose.yml`. Vérifié en conditions réelles : après recréation du conteneur, les 3
...
| `audit_log_write_failed` | `api/main.py` (middleware `access_log`) | — (aucune métrique dédiée aujourd'hui, limite assumée) | Les logs d'audit (table `audit_logs`) sont la seule trace de sécurité/RGPD de qui a fait quoi ; si leur écriture échoue silencieusement, il faut pouvoir le savoir malgré tout |
...
sécurité/RGPD (cf. `RapportE_3.md`, C9), pas du logging opérationnel au sens strict.
...
solutions clé en main compliquées, faites au plus simple". La stack étant déjà en place pour C11
(cf. `docs/MONITORING.md`), la réutiliser pour C20 évite de maintenir deux outils de suivi
différents pour un seul projet.
...
- `grafana/provisioning/alerting/rules.yml` — les 3 règles d'alerte de la section 2.
